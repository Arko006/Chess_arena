...
import { Trophy, Plus, Clock, Users, ArrowRight, Swords, ShieldCheck, UserCheck, Sparkles, CheckCircle2 } from 'lucide-react';
...
export default function TournamentsPage() {
  ...
  const [startAs, setStartAs] = useState<'PLAYER' | 'ARBITER'>('PLAYER');
  ...
  const openStartModal = (role: 'PLAYER' | 'ARBITER') => {
    if (!currentUser) {
      router.push('/login');
      return;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-800 pb-6 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Browse championships, register for matches, or start your own tournament as an Arbiter or Player.
          </p>
        </div>

        {/* Start Tournament Options (Arbiter vs Player) */}
        <div className="flex items-center gap-3 self-start sm:self-auto flex-wrap">
          <button
            onClick={() => openStartModal('PLAYER')}
            className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 text-white font-bold shadow-lg shadow-indigo-600/20 transition-all flex items-center gap-2 text-xs hover:scale-[1.02]"
          >
          ...
          <button
            onClick={() => openStartModal('ARBITER')}
            className="px-4 py-2.5 rounded-xl bg-[#1a2030] hover:bg-[#232c42] text-amber-300 border border-amber-500/30 font-bold transition-all flex items-center gap-2 text-xs hover:scale-[1.02]"
          >
            <ShieldCheck className="w-4 h-4 text-amber-400" />
            <span>Start as Arbiter</span>
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-[#141824] border border-gray-800 rounded-2xl p-12 text-center">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <button
              onClick={() => openStartModal('ARBITER')}
              className="px-4 py-2 rounded-xl bg-[#1f2638] hover:bg-[#28324a] text-amber-300 text-xs font-bold transition-colors flex items-center gap-2 border border-amber-500/30"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tournaments.map((t) => {
            const isMyTournament = currentUser && t.createdById === currentUser.id;
            return (
              <Link
                key={t.id}
                href={`/tournaments/${t.id}`}
                className="bg-[#141824] border border-gray-800 hover:border-gray-700 rounded-2xl p-6 flex flex-col justify-between transition-all hover:scale-[1.01] group shadow-xl relative overflow-hidden"
              >
                <div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#141824] border border-gray-700 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl relative">
            <h2 className="text-2xl font-black text-white mb-1">Start a Tournament</h2>
            <p className="text-xs text-gray-400 mb-6">
              Choose your participation role, set time controls, and generate pairings.
            </p>

            {/* Role Selection Tabs */}
            <div className="mb-6">
              <label className="block text-xs font-semibold text-gray-300 uppercase tracking-wider mb-2">
                Your Role in this Tournament
              </label>
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() => setStartAs('PLAYER')}
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    {startAs === 'PLAYER' && <CheckCircle2 className="w-3.5 h-3.5 text-indigo-400" />}
                  </div>
                  <p className="text-[11px] text-gray-400 leading-relaxed">
                    Host and compete! You're automatically enrolled in Round 1 roster as <strong className="text-indigo-300">{currentUser?.name || 'Player'}</strong>.
                  </p>
                </button>

                <button
                  type="button"
                  onClick={() => setStartAs('ARBITER')}
                  className={`p-3.5 rounded-2xl border text-left transition-all relative ${
                    startAs === 'ARBITER'
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    {startAs === 'ARBITER' && <CheckCircle2 className="w-3.5 h-3.5 text-amber-400" />}
                  </div>
                  <p className="text-[11px] text-gray-400 leading-relaxed">
                    Direct and officiate! Full Arbiter Console to pair rounds, manage players, and referee matches.
                  </p>
                </button>
              </div>
            </div>
            ...
            <form onSubmit={handleCreateTournament} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Rules, schedule, and guidelines..."
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-800">
                ...
                <button
                  type="submit"
                  disabled={creating}
                  className={`px-5 py-2.5 rounded-xl text-white text-xs font-bold shadow-lg transition-all disabled:opacity-50 flex items-center gap-2 ${
                    startAs === 'ARBITER'
                      ? 'bg-amber-600 hover:bg-amber-500 shadow-amber-600/20'
                      : 'bg-indigo-600 hover:bg-indigo-500 shadow-indigo-600/20'
                  }`}
                >
                  {creating ? 'Launching...' : startAs === 'ARBITER' ? 'Launch as Arbiter' : 'Launch as Player'}
                </button>
              </div>
            </form>
          </div>
